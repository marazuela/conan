...
**Zero rows; no code writes it today** — **Phase 0 (the gate / first consumer, built first per the 0→1
critical path) establishes the reusable open/close helper (§5); Phase 1/2/3 import it.** Every run opens a
row at start and closes it in a `finally`, even on exception.
...
> **Caveat (resolved 2026-06-04):** `CONCURRENTLY` requires a UNIQUE index on the matview — **confirmed live:
> `bc_candidates_appl_uidx (application_number)` exists**, so `REFRESH … CONCURRENTLY` is valid (no plain-
> `REFRESH` fallback needed). The RPC is invoked rather than issuing raw DDL over
...
`modal_workers/shared/bc_pipeline_runs.py` — the open/close helper. **Built in Phase 0** (its first consumer
on the critical path); Phase 1/2/3 import it (Phase 1 ships it itself only if built standalone before Phase 0):
```python
