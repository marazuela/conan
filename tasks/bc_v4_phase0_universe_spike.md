...
| `modal_workers/shared/bc_pipeline_runs.py` | **Reusable fail-loud helper** (`open_run`/`close_run`). Phase 0 is its **first consumer** (the 0→1 critical path), so it is **built here**; Phase 1/2/3 import it. `open_run(client, *, pipeline_name, snapshot_date) → run_id` (status `'running'`); `close_run(client, run_id, *, status, n_processed, n_failed, cost_usd=0, log, reason)` with `status ∈ {succeeded,partial,failed}`, called in a `finally`. Contract mirrors Phase 1 §5. |
