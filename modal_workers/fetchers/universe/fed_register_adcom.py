...
Each notice whose body/title parses to a valid meeting date becomes one
catalyst_universe row at profile=binary_catalyst, catalyst_type=adcomm.
Notices without a parseable date are counted as `skipped` (not errored) —
some Federal Register notices announce technical procedure or charter
renewals rather than a specific meeting date.
...
committee tag detection, drug-name candidate extraction, raw_text_url
body fetch) is reused from modal_workers.shared.fda_advisory_calendar
— the same helper that fda_pdufa_pipeline already uses to emit
`adcom_scheduled` signals. This fetcher is the catalyst_universe
complement to that signal path.
...
# Charter-renewal / withdrawal-hearing notices carry a charter-expiry date in
# the body (e.g. "Authority will expire on May 30, 2028") which the date regex
# happily matches. These are NOT meetings — they're announcements about the
# committee itself or post-approval enforcement. Reject by title shape.
_NON_MEETING_TITLE_RE = re.compile(
    r"\b(renewal|withdrawal|charter|information\s+collection|opportunity\s+for\s+a\s+hearing)\b",
    re.IGNORECASE,
)
    ...
    *,
    ...
    for m in meetings:
        ...
        if not m.meeting_date:
            ...
            skipped += 1
            continue
        if _NON_MEETING_TITLE_RE.search(m.title or ""):
            # Charter renewal / withdrawal-hearing / info-collection — the
            # date in the body refers to charter expiry or comment deadline,
            # not a committee meeting. The helper's body fetch surfaces these
            # dates indiscriminately, so we reject them at the writer.
            skipped += 1
            continue
...
def _map_meeting_to_row(m: Meeting) -> Dict[str, Any]:
    ...
    return {
        ...
        "raw_payload": {
            ...
            "title": m.title,
            "abstract": m.abstract,
            "agenda_excerpt": m.agenda_excerpt,
        },
    }
