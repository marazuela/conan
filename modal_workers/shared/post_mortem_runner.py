...
def _beta_binomial_blend(
    ...
    # Symmetric CI on logit, transformed back. For n_post >= 8 this is a fine
    # approximation; for smaller n the literature prior dominates anyway.
    import math
...
def _refit_reference_class(sb: SupabaseClient, reference_class: str) -> None:
    ...
    PR-1 (cross-cutting orchestrator fix): if an existing row carries
    source='literature' and the empirical cohort is below
    EMPIRICAL_TAKEOVER_THRESHOLD, we Beta-Binomial blend the empirical evidence
    with the literature prior and write source='blended'. Once the empirical
    cohort reaches the threshold, we take over fully with Wilson-CI and
    ...
    if (prior
        ...
        # Blend mode: posterior under literature prior + new empirical evidence.
        ...
        # Takeover (or no prior): Wilson-CI on empirical alone.
        rate = successes / n
