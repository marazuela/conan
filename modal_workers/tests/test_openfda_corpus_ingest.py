...
def test_scan_routes_to_deep_sweep_on_sunday():
    ...
    call = deep_mock.call_args
    assert call.kwargs["days"] == DEEP_SWEEP_DAYS
    # Wall-clock deadline is computed at scan-time (time.time() + budget) so
    # we only assert it's present and in the future, not the exact value.
    import time as _time
    assert call.kwargs["wall_clock_deadline"] > _time.time()
...
# ---------------------------------------------------------------------------
# Wall-clock soft timeout — graceful exit before Modal SIGKILL
# Added 2026-05-12 after the 2026-05-11 incident where the 900s Modal hard
# timeout SIGKILL'd mid-ingest, leaving 3408 docs half-written + scanner_runs
# status='error' + metrics=null.
# ---------------------------------------------------------------------------


def test_drugsfda_breaks_gracefully_at_wall_clock_deadline():
    """When wall_clock_deadline is in the past, the loop must bail BEFORE the
    next HTTP fetch, flag wall_clock_timeout_hit, and return everything
    written so far. Past pages must remain written."""
    import time as _time
    writer = _make_mock_writer()
    pages = [
        {"results": [_drugsfda_record(f"P0{i:03d}") for i in range(100)]},
        # Second page would normally be fetched; deadline kills the loop first.
        {"results": [_drugsfda_record(f"P1{i:03d}") for i in range(100)]},
    ]

    # Patch time.time so the deadline starts in the future, then snaps to the
    # past after the first page. We do this with a counter on _openfda_get.
    fetch_count = {"n": 0}
    deadline = _time.time() + 1000.0  # initial budget plenty
    real_time = _time.time

    def _ticked_time():
        # First two calls (pre-loop check + first iter check) return now.
        # After the first page is consumed, jump forward past the deadline.
        if fetch_count["n"] >= 1:
            return deadline + 1.0  # past deadline → next iter must break
        return real_time()

    def _get(path, params, **_):
        fetch_count["n"] += 1
        return pages[fetch_count["n"] - 1] if fetch_count["n"] <= len(pages) else None

    with patch.object(openfda_ingest, "_openfda_get", side_effect=_get), \
         patch.object(openfda_ingest, "time") as time_mock:
        time_mock.time.side_effect = _ticked_time
        # time.sleep is also imported via `time.sleep` in the http helper —
        # forward it to the real module so backoffs don't deadlock the test.
        time_mock.sleep = _time.sleep
        r = ingest_drugsfda_approvals(
            writer=writer, wall_clock_deadline=deadline,
        )

    # Only page 0 was fetched + ingested. Page 1 was skipped by the soft halt.
    assert r.wall_clock_timeout_hit is True
    assert r.documents_seen == 100
    assert writer.write_document.call_count == 100


def test_label_breaks_gracefully_at_wall_clock_deadline():
    """Same contract for the dailymed label feed."""
    import time as _time
    writer = _make_mock_writer()
    pages = [
        {"results": [_label_record(f"set_0_{i:03d}") for i in range(100)]},
        {"results": [_label_record(f"set_1_{i:03d}") for i in range(100)]},
    ]

    fetch_count = {"n": 0}
    deadline = _time.time() + 1000.0
    real_time = _time.time
...
def test_no_wall_clock_deadline_means_no_check():
    """Default behavior (deadline=None) must not introduce any time-based
    exits. Backward-compat for operator backfills that pass max_pages but
    no wall_clock_deadline."""
    writer = _make_mock_writer()
    pages = [{"results": [_drugsfda_record(f"A{i:03d}") for i in range(50)]}]

    with patch.object(openfda_ingest, "_openfda_get",
                      side_effect=_paged_responses(*pages)):
        r = ingest_drugsfda_approvals(writer=writer)  # no deadline

    assert r.wall_clock_timeout_hit is False
    assert r.documents_seen == 50


def test_scan_status_is_partial_when_wall_clock_truncated():
    """ScannerResult.status must be 'partial' (not 'error') when a feed bailed
    on wall-clock truncation but no actual errors occurred. The scanner_runs
    row then preserves the metrics instead of getting marked as a failure."""
    fake_drugsfda = openfda_ingest.IngestRunResult(
        documents_seen=100, documents_written=80, documents_dedup_hit=20,
        wall_clock_timeout_hit=True,
    )
    fake_label = openfda_ingest.IngestRunResult(documents_seen=0)
    ...
    assert result.run_metrics["wall_clock_truncated"] is True
    assert (result.run_metrics["feeds"]["drugsfda"]
            ["wall_clock_timeout_hit"]) is True
    # documents_written from the truncated feed must still surface — the
    # 2026-05-11 incident lost these by status='error' + metrics=null.
    assert result.run_metrics["documents_written_total"] == 80
