...
10. Narrative check — For any investment candidate, have I completed the mandatory web research layer? Does the market narrative match or conflict with my thesis? Have I searched for news, analyst activity, litigation, and sentiment that could strengthen, weaken, or kill the thesis?
...
* Web research is mandatory for all investment candidates. After strategy-specific analysis, use WebSearch to gather recent news, analyst commentary, litigation, regulatory actions, and market sentiment. This layer validates the thesis against the real-world narrative — our structured signal sources don't capture what the market is saying. Full research checklist is in the project files.
...
1. **Concurrency check** (scheduled sessions only): Read `SESSION_LOCK.md`. If it exists and the timestamp is less than 4 hours old, another session is still active — stop immediately, do nothing else. If it doesn't exist or the timestamp is stale (>4 hours), create/overwrite it with the current timestamp and proceed. Interactive sessions with the user present skip this step.
2. Read `SESSION_STATE.md` — the relay baton. Current phase, what's done, what's in progress, what's next, active warnings, active blockers. This is the single fastest path to full orientation.
3. Read `INSTRUCTIONS.md` — architecture, pipeline, session rules, execution environment, priority queue.
4. Read `OPEN_QUESTIONS.md` — if SESSION_STATE flags blockers.
5. Read only the task-specific file needed (strategy spec, scoring rubric, etc.).
...
5. Delete `SESSION_LOCK.md` to release the lock for the next scheduled session.

The test: if a new session reads SESSION_STATE.md and cannot determine exactly what to do next, the handoff has failed.

9. Maximum Utilization Rule — NO EXCEPTIONS

Work until the usage limit. Do not stop early. Do not conserve tokens. Do not "wrap up" when there is remaining capacity. Maximize productive output in every session. This project is designed for sustained autonomous work — use every available resource productively.

When one task completes, IMMEDIATELY start the next task from the priority queue or SESSION_STATE's next actions list. "I've completed the main task" is NOT a valid reason to stop — there is always a next task. The ONLY acceptable reason to stop before the limit is if ALL actionable work is genuinely blocked (in which case, document the blockers in `OPEN_QUESTIONS.md` and `SESSION_STATE.md`). Stopping early when unblocked work remains is a failure mode that wastes session capacity.
...
* Concurrency lock. Before doing any work, check `SESSION_LOCK.md`. If another session is active (lock exists, timestamp <4 hours old), stop immediately. If the lock is stale (>4 hours), override it. Always delete the lock during shutdown. This prevents two sessions from writing to the same files simultaneously.
...
This project is producer-only. Performance reports and deep-dive investment theses are generated by the project-root `Reporting Hub/` (tasks `reporting-hub-performance` and `reporting-hub-deep-dives`), which reads this system's state files (`SESSION_STATE.md`, `PROGRESS_LOG.md`, `candidates/`, `signals/`, `reports/`) and writes exclusively inside `Reporting Hub/`.

- Do NOT write to `Reporting Hub/` from this system. Ever.
- Do NOT recreate a `reporting_layer/` subfolder — it was intentionally removed on 2026-04-15.
- The hub's read contract is authoritative: `Reporting Hub/SOURCES.md`.

If the hub asks for a new state field or new file, add it inside `investment_discovery_system/` and register it in `SOURCES.md`.
