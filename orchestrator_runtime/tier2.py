...
def persist_tier2_assessment(
    ...
    *,
    ...
    row: Dict[str, Any] = {
        ...
        # Tier-2 is architecturally exempt from the Stage 7 constitutional gate
        # (constitutional_pass stays NULL per TIER2_FORBIDDEN_NON_NULL above).
        # gate_status='tier2_skipped' makes the skip explicit so downstream
        # callers can filter on gate_status='pass' without conflating Tier-2
        # emits with Tier-1 "not evaluated yet" rows.
