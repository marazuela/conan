...
## D-132 — v4 AI-first runtime cleanup and eval sidecar gate (2026-05-26)

**Context.** Review of the analysis stack found that Conan was not underusing AI broadly; it was overusing code around intermediate reasoning orchestration. The live v4 path had already collapsed much of the v3 chain, but the codebase still retained the `ORCH_V4=0` rollback branch, Stage 2 hypothesis module, Stage 3 premortem module, Stage 6 ensemble module, and semantic Stage 7 constitutional module. Those paths increased maintenance cost and kept stale tests/docs alive after the production path had moved to single-pass FDA + commercial synthesis.
...
## D-133 — v4 Phase 6 code-level close-out (2026-05-28)

**Context.** Phase 6a (flag flip), 6b (Tier-2 deletion), and 6c (v3 codepath removal) all landed in code between 2026-05-26 and 2026-05-27 (commits `2626554`, `fe98972`, `1e5bab9`). However the Cowork-side Phase 6b teardown (delete the `bulk_orchestrator_run` skill + wrapper + the two scheduled tasks) had not been done, leaving a critical operator_flag (`tier2_modal_actions_missing`, id `2cefc672`) firing on every Cowork bulk-orchestrator attempt because the Modal-side `tier2_*` actions were correctly deleted but the upstream caller kept retrying. A migration drift audit (PR #155) had also incidentally deleted the local provenance file `20260527000000_persist_assessment_v3_null_id_fix.sql`; the DDL is correctly applied on live, but the repo no longer documents the fix.

**Decision.** Close Phase 6 at the code level. Specifically: (1) delete `skills/bulk_orchestrator_run.md` + `wrappers/bulk_orchestrator_run.md` from `marazuela/conan-cowork-skills` main, strip the `bulk_orchestrator_priority1` SLA probe + escalation reference from `skill_watchdog.md` (both skill + wrapper) and the routine entry from `wrappers/README.md`; (2) delete the `~/.claude/scheduled-tasks/bulk_orchestrator_priority1` + `priority2` directories from the host filesystem; (3) resolve operator_flag `2cefc672` with a `resolved_note` pointing at this decision; (4) update `data_flow_diagram.md`, the PRD status line, and `DECISIONS.md` (this entry) to reflect that v4 is the only live runtime and Tier-2 is gone. Do NOT mark Gate 6 (50-asset operator review + final PRD stamp) closed — that remains a separate human gate. Do NOT forward-port the orphaned `feat/v4-foundation` hotfixes (`4a28765` reactor formatErrorForDlq, `032c43f` scanner liveness silent-filter): both were verified already on main HEAD, salvaged via PR #92.

**Consequences.** `marazuela/conan-cowork-skills` commit `181b9c6` deletes the Phase 6b Cowork surface. Operator_flag `2cefc672` is resolved. The only Phase-6-shaped item still open is the human Gate-6 review, blocked by an exhausted Anthropic credit balance (see `tasks/v4_phase6_close_blockers.md`) — zero `orchestrator_version='orch-v4.0'` rows exist on live yet, so the Day-1-after-6c verification (`docs/v4_phase6_runbook.md:316-322`) cannot run until credits restored. `feat/v4-foundation` is dead history.
