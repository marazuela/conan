...
**Multiplex compute endpoint addendum (2026-05-08, same-day after the foundation commit f19a982):** Phase 4B's last functional gap — Cowork → Modal HTTP bridge — was blocked on the Modal free-tier 8-endpoint cap. Resolved by shipping ONE `@modal.fastapi_endpoint(label="compute-v3")` on conan-v3-orchestrator that takes `{"action": "<name>", "args": {...}}` and dispatches internally to the runtime helpers. Adds 1 endpoint slot total (workspace cap is now 9; conan-v2 still uses 8, conan-v3 uses 1). Auth via `x-conan-compute-secret` mirrors the v2 pattern (constant-time compare, 401 on mismatch, 500 on server misconfig). Four actions wired: `tier2_bulk_enqueue`, `tier2_complete`, `tier2_fail`, `ic_memo_run`. The corresponding SQL wrappers (`rpc_tier2_bulk_enqueue`, `rpc_tier2_complete`, `rpc_tier2_fail`, `rpc_ic_memo_run`) all call `_conan_modal_post_enqueue('compute_v3', {action, args})` + pair with the existing `rpc_compute_collect(request_id, max_wait_ms)`. Operator step after deploy: `UPDATE internal_config SET value=<deployed compute-v3 URL> WHERE key='modal_url_compute_v3'` — the migration seeds an unreachable placeholder so accidental pre-seed RPC calls fail loudly with a pg_net transport error rather than routing somewhere unintended. With this in place, the Cowork `bulk_orchestrator_run` skill can retire its direct-insert fallback once the operator has run the `modal deploy` + the secret-update steps.
